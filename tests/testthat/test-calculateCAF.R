...
test_that("calculateCAF", {
  ...
  testthat::expect_equal(round(mean(caf$accPerSubject), 1),  0.9)
  ...
  testthat::expect_equal(length(unique(caf$bin)), 5)
  testthat::expect_equal(round(mean(caf$accPerSubject), 1),  0.8)
  ...
  testthat::expect_equal(round(mean(caf$accPerSubject), 1),  0.7)

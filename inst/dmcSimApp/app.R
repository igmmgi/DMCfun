  ...
  ui = shiny::fluidPage(
    shiny::sidebarLayout(
      shiny::sidebarPanel(width = 2, style = "margin: 10px; overflow-y:scroll; max-height: 10%; font-size:14px",
        ...
        shiny::sliderInput(inputId = "amp",             label = "amp",             min =     0,    max =  50,     value =     20,   ticks = FALSE, step = 1,    width = 200),
        ...
        shiny::sliderInput(inputId = "bnds",            label = "bnds",            min =     20,   max = 150,     value =     75,   ticks = FALSE, step = 5,    width = 200),
        ...
        shiny::sliderInput(inputId = "bnds_rate",       label = "bnds_rate",       min =     0,    max =   1,     value =      0,   ticks = FALSE, step = 0.01, width = 200),
        shiny::sliderInput(inputId = "bnds_saturation", label = "bnds_saturation", min =     0,    max = 500,     value =      0,   ticks = FALSE, step =    1, width = 200),
  ...
  server = function(input, output) {
    output$dmcSim <- shiny::renderPlot({
      ...
      dmc <- dmcSim(amp = input$amp, tau = input$tau, drc = input$drc, bnds = input$bnds,
        ...
        bnds_rate = input$bnds_rate, bnds_saturation = input$bnds_saturation,

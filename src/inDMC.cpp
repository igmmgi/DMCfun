...
void processInputArgs(int argc, char** argv, Prms &p, bool &argProblem) {
    ...
    try {
        while ((option = getopt_long(argc, argv, "", long_opts, &idxOption)) != -1) {
            switch (option) {
                case  0: p.amp = std::stod(optarg); break;
                case  1: p.tau = std::stod(optarg); break;
                case  2: p.aaShape = std::stod(optarg); break;
                case  3: p.mu = std::stod(optarg); break;
                case  4: p.bnds = std::stoi(optarg); break;
                case  5: p.resMean = std::stod(optarg); break;
                case  6: p.resSD = std::stod(optarg); break;
                case  7: p.nTrl = std::stoul(optarg); break;
                case  8: p.tmax = std::stoi(optarg); break;
                case  9: p.varSP = static_cast<bool>(std::stoi(optarg)); break;
                case 10: p.spShape = std::stod(optarg); break;
                case 11: p.spLimLow = std::stod(optarg); break;
                case 12: p.spLimHigh = std::stod(optarg); break;
                case 13: p.varDR = static_cast<bool>(std::stoi(optarg)); break;
                case 14: p.drLimLow = std::stod(optarg); break;
                case 15: p.drLimHigh = std::stod(optarg); break;
                case 16: p.drShape = std::stod(optarg); break;
                case 17: p.fullData = static_cast<bool>(std::stoi(optarg)); break;
                case 18: p.nTrlData = std::stoi(optarg); break;
                case 19: p.stepDelta = std::stoi(optarg); break;
                case 20: p.stepCAF = std::stoi(optarg); break;
                case 21: p.printInputArgs = static_cast<bool>(std::stoi(optarg)); break;
                case 22: p.printResults = static_cast<bool>(std::stoi(optarg)); break;
                case 23: p.setSeed = static_cast<bool>(std::stoi(optarg)); break;
                case 24:
                    showHelp();
                    argProblem = true;
                    break;
